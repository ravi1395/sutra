...
| Date | Version | Criterion | Surface | Evidence | Status |
|---|---|---|---|---|---|
| 2026-07-14 | 2.3.2 | CLI-1: Install CLI visibly handles administrator fallback and installed shim forwards a path | UI/shell | User-driven live verify on v2.3.2 dev build: Install CLI dialog showed full command + Copy/Close, Copy wrote exact command, shim installed, `command -v sutra` resolved, `sutra <path>` opened/focused the path. | PASS |
| 2026-07-14 | 2.3.2 | CLI-2: installed CLI returns immediately and Sutra survives terminal `Ctrl-C`/close | shell/process | User-driven live verify: `sutra .` returned immediately, opened/focused workspace; Ctrl-C and terminal close both left Sutra running. | PASS |
...
| Date | Version | Criterion | Surface | Evidence | Status |
|---|---|---|---|---|---|
| 2026-07-14 | 2.3.2 | DBG-6: floating control strip appears centered over the editor pane on Start, disappears on Stop/adapter death — no leftover pill, no layout shift underneath | UI | User-driven live verify against debugpy/loop.py fixture: pill appeared on Start, fully gone on Stop, no layout shift. | PASS |
| 2026-07-14 | 2.3.2 | DBG-7: statusbar debug chip shows `debug: running` while running and `paused · <file>:<line>` while paused, positioned in the whisper bar right after the diagnostics chip; clicking it while paused jumps the editor to that frame | UI | User-driven live verify: chip showed `debug: running`, switched to `paused · loop.py:<line>` on breakpoint hit, click jumped editor to that frame. | PASS |
| 2026-07-14 | 2.3.2 | DBG-8: palette (⌘⇧P, `>`) lists Debug: Continue/Pause/Step Over/Step Into/Step Out/Stop only while a session is live, Debug: Start always listed; every listed command's effect matches its F-key | UI | User-driven live verify: no-session palette showed only Debug: Start; live-session palette showed all seven; palette Continue matched F5 effect. | PASS |
