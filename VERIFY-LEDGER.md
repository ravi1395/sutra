...
| 2026-07-16 | 2.3.3 | TURN-10: PRE-EXISTING (found by re-gate, not a rehaul regression): turn test chip never live-updates — onRunnerDone records via turnTestRecord IPC but never re-fetches turnList, chip appears only after rollback/reopen | UI | — | BLOCKED(auto-test on turn close → does pass/fail chip appear on the turn row without a rollback or workspace reopen? if not, file follow-up fix) |
