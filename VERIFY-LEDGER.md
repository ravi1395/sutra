...
| Date | Version | Criterion | Surface | Evidence | Status |
|---|---|---|---|---|---|
| 2026-07-21 | 2.3.4 | TERM-1: in a native Sutra terminal, Vim receives `i`, `Esc`, `:q!`, `Tab`, and F1–F12 before and after terminal hide/show, North drawer open/close, tab activation, and app focus loss/return; Prompt Builder, task Start, and Automations still deliver once through their existing terminal seams | UI+PTY | Partial native PASS against isolated `Sutra Verify Terminal.app` built from `d279f03`: Vim `i` + text + `Tab` + `Esc` + `:q!` produced `/tmp/sutra-vim-d279`; the same sequence after terminal hide/show produced `/tmp/sutra-vim-d279-restore`; app focus loss/return produced `/tmp/sutra-vim-d279-appfocus`; `cat` captured all F1–F12 xterm sequences. Prompt Builder, Tasks, and Automations surfaces opened normally; focused compatibility tests passed 119/119 and full suite 765/765. 2026-07-21 second pass (same `d279f03` bundle, computer-use clicks + seeded `.sutra` files + fake `is_agent` terminal — ad-hoc-signed `cat` copy named `claude` running as app PTY child pid 98146): Automations delivery observed — "TERM1 echo" run wrote exactly one `TERM1-AUTO` line to `/tmp/sutra-term1-auto.txt` (wc -l = 1, command visibly executed once in zsh 1); task Start delivery observed — seeded READY task delivered `^[[200~TERM1-TASK-DELIVERY-MARKER^[[201~` exactly once (grep -c = 1) to the fake agent PTY via Submit; North-view terminal drawer hide/show toggled via btn-term with zsh 3 content intact on restore. 2026-07-21 user-confirmed live: Vim raw keys around North drawer open/close, around tab activation, and a typed Prompt Builder send all delivered correctly ("looks good"). | PASS |
| 2026-07-21 | 2.3.4 | FOCUS-1: clicking the terminal card padding/dead strip (outside `.xterm`) focuses xterm — Vim receives keys immediately, no tab-click needed | UI | Fix `src/terminal.ts` wrapper mousedown `preventDefault` (WebKit default focus resolution was undoing `textarea.focus()`); mechanism root-caused + observed live 2026-07-21 by Fable subagent. Round 1 `preventDefault`; round 2 added click-phase re-assert (tab-onclick timing) scoped to clicks outside `.xterm`. NOTE: the 2026-07-21 "hollow cursor after padding click" live FAIL was misattributed — user's screenshot showed the click hit the light `#main` margin gap BELOW the North terminal card (see FOCUS-3), not the in-card wrapper padding; the wrapper fix itself is still unverified live. Contract tests `tests/terminal-input.test.ts` green; suite 774/774. 2026-07-21 user-confirmed live: dark in-card padding click focuses Vim, keys register. | PASS |
| 2026-07-21 | 2.3.4 | FOCUS-3: clicking dead chrome around the North terminal card — the 10px `#main` margin gap, `#north-seam` / `#whisper-bar` background between chips — leaves terminal focus intact; typing still reaches the PTY | UI | Root cause: North `#terminal-area` has `margin: 0 10px 10px`, so clicks in the light gap target bare `#main`; no handler owned them and WebKit resolved focus to body (hollow cursor observed live 2026-07-21). Fix: dead-zone guard in `src/main.ts` — self-target-only mousedown `preventDefault` on `#main`, `#whisper-bar`, `#north-seam` (children unaffected). Contract test in `tests/terminal-input.test.ts`; suite 774/774; tsc clean. 2026-07-21 user-confirmed live: white margin-strip click no longer steals focus, Vim keys register. | PASS |
| 2026-07-21 | 2.3.4 | FOCUS-2: dismissing the command palette by click-away (or Esc/Enter/toggle) returns focus to the pre-open owner — terminal keeps receiving PTY keys, no stranded-on-body state routing keys to global shortcuts | UI | Fix `src/palette.ts` capture-on-open + `restorePaletteFocus` on close, wired through the drawer `recoverFocus` contract (`terminals.focusActive()` for terminal-owned targets) in `src/main.ts`; symptom (Vim splash never clearing after palette click-away) observed live 2026-07-21. Skeptical-review gate caught click-away's own mousedown re-stranding focus post-restore → backdrop-scoped `preventDefault` added in the same change. Unit + contract tests in `tests/palette.test.ts` green (6 new); suite 772/772; tsc clean. 2026-07-21 user-confirmed live: Vim receives keys after palette dismissal ("vim looks good now"). | PASS |

## Multi-view cleanup + North/Stanza bug fixes (uncommitted, session 2026-07-22)

Two North ledger bugs + Stanza outline race fixed; four "align with classic" toggle-redundancy decisions applied (user-chosen). Logic double-gated: 7-way parallel root-cause triage + skeptical-review (1 GATE FAIL on a search exit-leak → fixed + re-reviewed). tsc clean, 778 npm tests pass (+4). Rows below are the live-window criteria code trace + node:test cannot observe — all BLOCKED pending user live `npm run tauri dev` confirmation.

| Date | Version | Criterion | Surface | Evidence | Status |
|---|---|---|---|---|---|
| 2026-07-22 | post-2.3.4 | MVX-1: North ledger scroll no longer snaps to top on the 1.5s agent poll — scroll a ledger with several turns to the bottom, wait through ≥2 polls, position holds; a new turn arriving preserves scroll | UI | `src/ledger.ts` render() signature guard (`JSON.stringify({root,empty,rows})`) skips identical rebuilds; scrollTop captured/restored on real rebuilds. Tests `tests/ledger.test.ts` (skip-rebuild + restore-across-rebuild). Live-scroll unobserved | PASS (user-confirmed live 2026-07-22) |
| 2026-07-22 | post-2.3.4 | MVX-2: North ledger Review-diff (and row summary) omit files touched-then-reverted within a turn (beforeHash===afterHash); added/deleted/modified still shown | UI | `turnFileChanged()` filter in `refreshScopedDiffFileList` + `ledgerRenderModel` fileNames; tests `turnFileChanged`, `ledger row lists only net-changed files`. Real reverted-file turn unobserved | PASS (user-confirmed live 2026-07-22) |
| 2026-07-22 | post-2.3.4 | MVX-3: cold-launch into Stanza → outline panel is populated for the restored active file WITHOUT a room switch | UI | `outlineView.onActiveFileChanged()` after `restoreWorkspaceTabs` in `openWorkspace`. RISK (skeptical-review 3c): async symbol fetch may still race a not-yet-built index on a large workspace — narrows, may not fully close. Needs cold-load observation | PASS (user-confirmed live 2026-07-22) |
| 2026-07-22 | post-2.3.4 | MVX-4: in Stanza, titlebar term/diff/browser toggles are hidden (rooms drive them); composer toggle remains; switching rooms shows/hides the right surfaces | UI | `styles.css` `.view-stanza` hides `#btn-term/#btn-diff/#btn-browser`; `#btn-composer` kept (no room preset enables composer, per `rooms.ts` ROOM_PRESETS). Live titlebar unobserved | PASS (user-confirmed live 2026-07-22) |
| 2026-07-22 | post-2.3.4 | MVX-5: Stanza Write-room shelf shows Files+Outline only (no Search section); ⇧⌘F swaps to search and back to the stacked shelf cleanly; switching Stanza→Classic/Graphite with search open does NOT leave tree+search double-rendered | UI | `sidebarSections("stacked")`→["files","outline"]; `syncShelfMode` closes search on every shelf transition (entry+exit); openSearchView/closeSearchView suspend/resume shelf. `tests/shelf.test.ts` pins order; exit-leak fix + transitions unobserved live | PASS (user-confirmed live 2026-07-22) |
| 2026-07-22 | post-2.3.4 | MVX-6: in North, the titlebar file-tree button (📁) toggles the drawer, and ⌘B also toggles it; ⌘E still works; button absent in other views | UI | `index.html` #btn-sidebar (North-only via CSS) wired to `setSidebar`; ⌘B handler no longer excludes north (drawer branch). Non-north ⌘B byte-identical to prior. Live toggle unobserved | PASS (user-confirmed live 2026-07-22) |
| 2026-07-22 | post-2.3.4 | MVX-7: in Graphite, the titlebar terminal button is hidden and the band tabs (Terminal/Problems) still toggle the terminal; ⌘J still works | UI | `styles.css` `.view-graphite` hides `#btn-term`; band-tab + setTerminal graphite branch key off `isGraphite`, not button. Live unobserved | PASS (user-confirmed live 2026-07-22) |

## Round 2 — live-smoke feedback fixes (uncommitted, 2026-07-22)

User live smoke: North Light all PASS. Remaining/new items below.

| Date | Version | Criterion | Surface | Evidence | Status |
|---|---|---|---|---|---|
| 2026-07-22 | post-2.3.4 | MVX-3b: Stanza outline populates on cold launch AND on manual file-select (supersedes MVX-3). Real cause: `activateInPane` fires `onActiveTabChanged`→outline refresh BEFORE `langDidOpen` registers the doc (editor.ts:1492 vs :1495), so `getDocumentSymbols` always hit an unregistered doc. Fix: editor fires `onDocumentParsed(path)` after langDidOpen resolves; main.ts re-refreshes the outline if that path is still active | UI | editor.ts onDocumentParsed + main.ts wiring; tsc clean, 778 tests. NO unit seam (async IPC ordering). Discriminating Q attached to handback: "No symbols" text vs no-outline-area vs unsupported lang | PASS (user-confirmed live 2026-07-22) |
| 2026-07-22 | post-2.3.4 | MVX-8: in Stanza, non-Run rooms (Write/Review/Web) show NO terminal — not even the 30px collapsed "terminal · N threads" seam strip; the Run room still shows the terminal | UI | Root: `setTerminal(false)` collapses to a 30px seam; Graphite hides it, Stanza didn't. Fix `styles.css` `:root.view-stanza #terminal-area.terminal-collapsed { display:none }`. Live unobserved | PASS (user-confirmed live 2026-07-22) |
| 2026-07-22 | post-2.3.4 | MVX-9: clicking Problems shows the diagnostics list (never a blank pane) — empty state renders "No problems found."; in Graphite the Problems tab fills the drawer footprint (~40%) instead of collapsing to a ≤200px strip | UI | `diagnostics.ts` empty-state row; `styles.css` `:root.view-graphite #problems-panel-host:not(.hidden){flex:0 1 40%;max-height:none}`. #3b is a blind graphite layout change — MUST live-verify. tsc clean, 778 tests | PASS (user-confirmed live 2026-07-22) |
