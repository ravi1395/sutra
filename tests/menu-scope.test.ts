...
test("no menu row is duplicated across the two menus", () => {
  const ws = new Set<string>(WORKSPACE_MENU_VERBS);
...
test("settings and updates live only in the app menu", () => {
  ...
  assert.ok(!(WORKSPACE_MENU_VERBS as readonly string[]).includes("settings…"));
  assert.ok(!(WORKSPACE_MENU_VERBS as readonly string[]).includes("check for updates…"));
});

test("open folder lives only in the workspace menu", () => {
  assert.ok(WORKSPACE_MENU_VERBS.includes("open folder…"));
  assert.ok(!(APP_MENU_VERBS as readonly string[]).includes("open folder…"));
});
