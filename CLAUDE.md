...
## State
- Version: v2.3.0 — bump all 3 in lockstep: `package.json:4`, `src-tauri/Cargo.toml:3`, `src-tauri/tauri.conf.json:4` (+ `package-lock.json` / `src-tauri/Cargo.lock`). Update this line every bump. **v2.3.0 = internal cleanup** (dead-field removal in proxy::Head); no new user-facing surface. **v2.2.0 = UX refinement** (unified ⌘P palette, single-home menus, version-pill→What's New badge, About consolidation), merged + tagged; 7 pure-UI acceptance criteria still owe a live `sutra-verify` pass. `feat/multi-window` merged in v2.1.1 (fe22292) — process-spawn/focus-IPC/Dock all shipped.
