...
## State
- Version: v2.2.0 — bump all 3 in lockstep: `package.json:4`, `src-tauri/Cargo.toml:3`, `src-tauri/tauri.conf.json:4` (+ `package-lock.json` / `src-tauri/Cargo.lock`). Update this line every bump. **v2.2.0 = UX refinement** (unified ⌘P palette, single-home menus, version-pill→What's New badge, About consolidation); merged + tagged, but 7 pure-UI acceptance criteria (palette feel A1/A3/A4/A6, badge visuals C2, `<500ms` perf A7) still need a live `sutra-verify` pass — code paths reviewed sound (GATE PASS). **`feat/multi-window` is still UNMERGED** — code-complete + unit-tested but needs a manual app smoke pass (process-spawn/focus-IPC/Dock) before merging; it will take the **next** version (v2.3.0), not 2.2.0.
