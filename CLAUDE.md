...
Knowledge graph at `graphify-out/` covers the whole codebase. Use it to answer questions *without* reading files. (`graphify-out/` is gitignored — in a fresh clone/remote session it is absent and the `graphify` CLI may not be installed; in that case skip this section and use rg/Read directly.)
...
src/
  ...
  composer.ts          docked prompt-composer panel (template picker, section inputs, chip rail)
  composer-store.ts    per-workspace draft + prompt history (localStorage)  |  composer-layout.ts  DOM-free layout helpers
  composer-complete.ts ranking/token logic for <task> CM6 completions  |  composer-files.ts  per-root autocomplete file scans
  prompt-builder.ts    composer sections + chips → XML-tagged prompt  |  prompt-tags.ts  tag/template schema, trust-gated loader
  tag-manager.ts       tag config editor (pure mutations + IPC persistence + modal)
  delivery.ts          composed prompt → PTY stdin (bracketed paste DECSET 2004)
  annotations.ts       parent-side canonical annotation state  |  annotation-core.ts  DOM-independent stable-selector helpers
  annotation-agent.ts  in-iframe IIFE (esbuild-bundled, no runtime app imports)  |  annotation-context.ts  size-capped context for prompts  |  annotation-store.ts  .sutra/annotations.json persistence
  tasks.ts             durable workspace task metadata  |  tasks-panel.ts  task panel (explicit create/start)
  agent-profiles.ts    project-local agent profile schema  |  worktree-dispatch.ts  local-only worktree dispatch dialog
  handoff.ts           pure handoff-candidate computation (files a task's linked turns touched)
  ledger.ts            North turn ledger model (phase / per-turn test state / rollback)
  rooms.ts             Stanza room router (RoomId → fixed surface preset)  |  surface-state.ts  SurfaceId/SurfaceState types
  north-seam.ts        North collapsed-seam surface state  |  drawer.ts  Focus/sidebar drawer host contracts
  popovers.ts          open-popover registry; window-blur dismissal (see v2.3.44)
  format.ts            beautify-on-save (prettier + @taplo/lib TOML)  |  format-ext.ts  extension eligibility, dependency-free
  terminal-input.ts    pure classifier for xterm onData chunks (typed text vs paste/control)
  theme-tokens.ts      theme-token helpers for non-CSS consumers (xterm, canvas)
  turn-actions.ts      deps-injected turn header actions wiring
...
src-tauri/src/
  ...
  assets.rs            scans .claude dirs (user + project) for skills/subagents/slash-commands
  proxy.rs             loopback proxy for in-app browser (target parse, loopback-only, cookie/token auth)
  dock_menu.rs         macOS Dock right-click menu (recents + New Window) — spike, see State
...
- Version: v2.3.44 — bump all 3 in lockstep: `package.json:4`, `src-tauri/Cargo.toml:3`, `src-tauri/tauri.conf.json:4` (+ `package-lock.json` / `src-tauri/Cargo.lock`). Update this line every bump. **v2.3.44 = dropdown/popover close-on-click-away + close-on-blur fix** (root cause 1: `terminal.ts` wrapper mousedown called `e.stopPropagation()`, killing bubble to `document` before the outside-click listeners on every open dropdown/popover/modal — gitbar, palette, contextmenu, breakpoint popover, settings/about, workspace menu, automations, open-editors menu — could see the click; replaced with a `__termInstanceHandled` flag the group-body handler checks, preserving the redundant-render skip without blocking propagation. root cause 2: nothing in the app ever closed on window blur — new `src/popovers.ts` open-popover registry + a `window.addEventListener("blur", ...)` in `main.ts` dismiss everything on app focus loss. 779 npm / 306 cargo green; user live-confirmed both click-away and blur-away in a running `tauri dev` session 2026-07-24; tagged + released.) **v2.3.43 = preview-tab fix + CLI install relocation** (bug: `Pane.renderTabs()` collapsed the whole tab strip to a single "Preview: X" entry whenever a tab's preview was toggled on, hiding every sibling tab until preview was closed — `tab.previewMode` was already tracked correctly per-tab in `activate()`, so the fix folds the previewed tab into the normal per-tab loop instead of replacing the strip; also fixed `closePreview()` not clearing `previewMode` (mouse-× and ⌘⇧V-off paths were inconsistent) and restored the agent/`prompt_user` synthetic preview tab the old branch also rendered (caught by skeptical-review, fixed same session). CLI install moved from the workspace wordmark popover into Settings → About, reusing `runCliInstall()`. Opus skeptical-review gated PASS on code/tests (778 npm / 306 cargo); live-GUI confirmation in a running app not completed this session — automation attempt was aborted for safety (stray keystrokes risked hitting a live terminal panel) before it could screenshot the fix working.) **v2.3.42 = vim/alt-screen terminal freeze fix** (production-only: vite default-target minify dropped the `let` in xterm 6.0.0's `requestMode` enum init → strict-mode ReferenceError on vim's first DECRQM query (mode 2026) → xterm write queue wedged, screen frozen while input still reached the PTY; fix = `vite.config.ts` `build.target: "esnext"` — load-bearing, keeps `||=` untransformed, re-check minified bundle for `let i;` before the NOT_RECOGNIZED enum IIFE if vite/xterm are ever upgraded; root-caused via TEMP keylog instrumentation in an instrumented release bundle, user live-verified vim insert PASS 2026-07-22). **v2.3.41 = multiview toggle cleanup + North/Stanza/Graphite fixes** (Stanza outline populates on open/switch — root cause was `onActiveTabChanged`/outline-refresh firing before `langDidOpen` registered the doc, fixed via new `editor.onDocumentParsed`; rooms are the sole surface control — redundant term/diff/browser titlebar toggles hidden via CSS, composer kept, non-Run rooms hide the collapsed terminal seam; Search removed from the stanza Write shelf → Files+Outline only, ⇧⌘F suspends/resumes stacked; North ledger render() signature-guarded to preserve scroll + `turnFileChanged` prunes reverted files from diff/row; North file-tree `#btn-sidebar` + ⌘B drawer toggle; Graphite drops the duplicate `#btn-term` + Problems tab fills the drawer; Problems empty-state "No problems found."; skeptical-gate caught 1 exit-leak regression → fixed; VERIFY-LEDGER MVX-1..9 all user-confirmed PASS 2026-07-22; tagged + released). **v2.3.4 = terminal raw-input + focus-steal fixes** (raw modal input — Esc/Tab/F1–F12 — stays in terminal instead of app shortcuts; centralized focus recovery across hide/show, tab activation, app focus return; WKWebView focus-steal fixes: terminal wrapper padding clicks — preventDefault + click-phase re-assert, palette dismissal capture/restore via drawer recoverFocus contract, dead-chrome guard on `#main`/`#whisper-bar`/`#north-seam` self-target mousedowns; VERIFY-LEDGER fully clear — TERM-1 + FOCUS-1/2/3 all user-confirmed PASS 2026-07-21; tagged + released). **v2.3.3 = multiview + branch-review scope** (North Light/Graphite/Stanza views w/ day/night themes, merge-base "vs main" diff scope, token restyle, annotation rail, turn-UX rehaul; ledger burned 24 BLOCKED → 106 PASS). **v2.3.2 = CLI installer visibility + safe argument forwarding + detached terminal launch**. **v2.3.1 = agent control plane + inline preview** (task panel, worktree dispatch, annotations, handoff, ⇧⌘V inline render — retires split preview pane). **v2.3.0 = internal cleanup**. **v2.2.0 = UX refinement**. `feat/multi-window` merged in v2.1.1 (fe22292) — process-spawn/focus-IPC/Dock all shipped.
...
- Tests: `npm test` → 779 pass (verified 2026-07-31); `cargo test` (inside src-tauri/) → 306 pass as of v2.3.44. `cargo test` needs the Tauri system libs (GTK/`gdk-3.0` etc. on Linux) — it will not build on a bare container without them.
