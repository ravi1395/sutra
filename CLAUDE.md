...
- Version: v2.3.4 — bump all 3 in lockstep: `package.json:4`, `src-tauri/Cargo.toml:3`, `src-tauri/tauri.conf.json:4` (+ `package-lock.json` / `src-tauri/Cargo.lock`). Update this line every bump. **v2.3.4 = terminal raw-input fix** (raw modal input — Esc/Tab/F1–F12 — stays in terminal instead of app shortcuts; centralized focus recovery across hide/show, tab activation, app focus return; VERIFY-LEDGER TERM-1 partial native PASS, North-drawer + delivery-seam proof still owed, not yet tagged/pushed). **v2.3.3 = multiview + branch-review scope** (North Light/Graphite/Stanza views w/ day/night themes, merge-base "vs main" diff scope, token restyle, annotation rail, turn-UX rehaul; ledger burned 24 BLOCKED → 106 PASS). **v2.3.2 = CLI installer visibility + safe argument forwarding + detached terminal launch**. **v2.3.1 = agent control plane + inline preview** (task panel, worktree dispatch, annotations, handoff, ⇧⌘V inline render — retires split preview pane). **v2.3.0 = internal cleanup**. **v2.2.0 = UX refinement**. `feat/multi-window` merged in v2.1.1 (fe22292) — process-spawn/focus-IPC/Dock all shipped.
